...
class CountryValidation(Validation):
  ...
  def validate_custom_country_update(country):
        ...
        if CountryValidation().validate_string(country['visum_regulations']['value'], min_length=0, max_length=50):
            country['visum_regulations']['errors'].append(f", {CountryValidation().validate_string(country['visum_regulations']['value'], 0, 50)}")
        ...
        if CountryValidation().validate_string(country['general_information']['value'], min_length=0, max_length=100):
            country['general_information']['errors'].append(f", {CountryValidation().validate_string(country['general_information']['value'], 0, 50)}")
